...
- crates.io publication is prohibited until released Git-pinned consumers have source-bound standalone/rollback and public artifact evidence (RUST-016), and all required migration, Rust/Go native and security gates are green. After those gates, a separately approved release must add API review, `cargo semver-checks`, provenance/SBOM setup and external registry ownership/read-back evidence (RUST-014). Registry-pinned consumer releases follow (RUST-015); they cannot be a prerequisite of first publication.
