...
- Rust crates do not receive a second repository tag. The manifest's `publish_order` is the only Rust publication ordering; it is not a tag namespace.
- Every workspace package is classified in the manifest. Only adopted crates may appear in `publish_order`; private test-support crates and non-adopted crates remain non-publishable.
...
2. validates manifest schema, adoption evidence, publish order and fail-closed publication state;
...
Local RUST-014 tooling is complete and wired into both normal CI and the tag release workflow. The following are deliberately **not** claimed here:
