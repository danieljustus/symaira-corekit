...
## Candidate-source scope decision — 2026-09-20

The frozen candidate manifest previously enforced its complete forensic source
set, including `.github/workflows/ci.yml`, `.gitattributes` and every unrelated
Rust crate. A workflow-only edit therefore failed 10 of 74 acceptance tests with
`frozen source manifest is not bound to the current candidate source`, and
lockfile/workflow dependency bumps could only merge after a full Go/Rust
recapture. That failure was scope drift: none of those files can change the
built example's bytes or the compared observations.

The accepted decision ([ADR RUST-003](adr-rust-003-candidate-source-scope.md))
keeps the recorded superset but binds verification to the enforced port inputs:
the workspace build inputs (`Cargo.toml`, `Cargo.lock`, `rust-toolchain.toml`,
`.cargo/**`), the crate under test (`rust/symaira-core-sqlite/**`), its
workspace path dependencies (`rust/symaira-core-fs/**`) and the Go oracle,
migration corpus and harness (`scripts/rust-port/sqlite/**`). Lockfile and
toolchain changes still require a recapture because they change the compiled
artifact; harness files stay enforced so acceptance tests cannot be weakened
without a reviewed re-freeze. `validate_scope()` walks the transitive path
dependencies of the crate under test and rejects a manifest that does not cover
them, so a new workspace path dependency forces a scope decision.
...
is retained unchanged as historical evidence for its own source snapshot. The
new manifest and capture were generated by the same process that changed the
harness, so they require independent review before they count as acceptance
