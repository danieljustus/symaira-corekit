...
// TestGenerateSendsImages verifies that GenerateOptions.Images are forwarded\n// as a base64 array in the request body, and that an empty/nil Images slice\n// does not produce an \"images\" key (backward-compatible for text-only callers).\nfunc TestGenerateSendsImages(t *testing.T) {\n\tfor _, tt := range []struct {\n\t\tname       string\n\t\timages     []string\n\t\twantImages bool\n\t}{\n\t\t{name: \"nil\", images: nil, wantImages: false},\n\t\t{name: \"empty\", images: []string{}, wantImages: false},\n\t\t{name: \"with_images\", images: []string{\"aW1hZ2U=\", \"aW1hZ2Uy\"}, wantImages: true},\n\t} {\n\t\tt.Run(tt.name, func(t *testing.T) {\n\t\t\tvar gotBody map[string]any\n\t\t\tserver := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {\n\t\t\t\tbody, _ := io.ReadAll(r.Body)\n\t\t\t\t_ = json.Unmarshal(body, &gotBody)\n\t\t\t\tw.Header().Set(\"Content-Type\", \"application/x-ndjson\")\n\t\t\t\tfmt.Fprintln(w, `{\"model\":\"m\",\"response\":\"ok\",\"done\":true}`)\n\t\t\t}))\n\t\t\tdefer server.Close()\n\n\t\t\tc := New(Config{BaseURL: server.URL, Model: \"m\"})\n\t\t\terr := c.Generate(context.Background(), \"\", \"prompt\", &GenerateOptions{Images: tt.images}, func(GenerateResponse) error {\n\t\t\t\treturn nil\n\t\t\t})\n\t\t\tif err != nil {\n\t\t\t\tt.Fatalf(\"expected success, got %v\", err)\n\t\t\t}\n\n\t\t\t_, hasImages := gotBody[\"images\"]\n\t\t\tif hasImages != tt.wantImages {\n\t\t\t\tt.Errorf(\"images key presence = %v, want %v (body keys: %v)\", hasImages, tt.wantImages, gotBody)\n\t\t\t}\n\t\t\tif tt.wantImages {\n\t\t\t\timgs, ok := gotBody[\"images\"].([]any)\n\t\t\t\tif !ok || len(imgs) != len(tt.images) {\n\t\t\t\t\tt.Errorf(\"expected %d images, got %v\", len(tt.images), gotBody[\"images\"])\n\t\t\t\t}\n\t\t\t}\n\t\t})\n\t}\n}
