// Package installmethod detects how a binary was installed to determine
...
// toolName is the display name of the binary (e.g. "symvault").
func Guidance(method InstallMethod, toolName string) string {
	switch method {
	case DirectDownload:
		return "Re-run the quick install script or download the latest release from GitHub"
	case Homebrew:
		return "Update via Homebrew: brew update && brew upgrade " + toolName
	case GoInstall:
		return "Update via Go: go install github.com/danieljustus/" + toolName + "@latest"
	case PackageManager:
		return "Update via your system package manager (e.g., apt upgrade, yum update, pacman -Syu)"
	case BuildFromSource:
		return "Rebuild from source: git pull && go build"
	case Unknown:
...
// GuidanceMethod returns actionable upgrade instructions for the given method
// without requiring a toolName. Uses a generic message.
func GuidanceMethod(method InstallMethod) string {
	return Guidance(method, "tool")
}
...
func detectFromReceipts(absPath string) InstallMethod {
	...
	// Check for dpkg receipts generic enough for any tool.
	// Uses the last two path components as fallback.
	base := filepath.Base(absPath)
	if _, err := os.Stat("/var/lib/dpkg/info/" + base + ".list"); err == nil {
		return PackageManager
	}
