...
func TestDetectHomebrewPaths(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Apple Silicon", path: "/opt/homebrew/bin/tool"},
		{name: "Intel Cellar", path: "/usr/local/Cellar/tool/1.0.0/bin/tool"},
		{name: "Linuxbrew", path: "/home/linuxbrew/.linuxbrew/bin/tool"},
	}
...
func TestDetectDirectDownloadPaths(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "user local bin", path: "/usr/local/bin/tool"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			method, err := Detect(tt.path)
...
func TestDetectPackageManagerPath(t *testing.T) {
	...
	method, err := Detect("/usr/bin/tool")
	if err != nil {
...
func TestDetectGoInstallFromGopath(t *testing.T) {
	...
	goBin := filepath.Join(home, "go", "bin", "tool")
...
func TestDetectEnvVars(t *testing.T) {
	...
	t.Run("GOPATH env", func(t *testing.T) {
		binary := filepath.Join(tmp, "bin", "tool")
	...
	t.Run("layered order env over path", func(t *testing.T) {
		binary := filepath.Join(tmp, "opt", "homebrew", "bin", "tool")
		if err := os.MkdirAll(filepath.Dir(binary), 0755); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(binary, []byte("fake"), 0644); err != nil {
...
func TestDetectGoCacheMarkers(t *testing.T) {
	...
	method, err := Detect("/go/pkg/mod/github.com/test/tool@v1.0.0/bin/tool")
	if err != nil {
		t.Fatalf("Detect() error = %v", err)
	}
	if method != GoInstall {
...
func TestDetectGoInstallAtSign(t *testing.T) {
	...
	method, err := Detect("/tmp/gopath/pkg/mod/example.com/tool@latest/bin/tool")
	if err != nil {
		t.Fatalf("Detect() error = %v", err)
	}
	if method != GoInstall {
...
func TestDetectUserLocalBins(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "home bin", path: filepath.Join(home, "bin", "tool")},
		{name: "local bin", path: filepath.Join(home, ".local", "bin", "tool")},
		{name: "cargo bin", path: filepath.Join(home, ".cargo", "bin", "tool")},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			method, err := Detect(tt.path)
...
func TestDetectHomebrewSymlink(t *testing.T) {
	...
	realBin := filepath.Join(tmp, "Cellar", "tool", "1.0.0", "bin", "tool")
	...
	symlinkPath := filepath.Join(tmp, "bin", "tool")
...
func TestDetectNonExistentPath(t *testing.T) {
	...
	method, err := Detect("/nonexistent/tool")
	if err != nil {
		t.Fatalf("Detect() error = %v", err)
	}
	if method != Unknown {
...
func TestGuidance(t *testing.T) {
	...
	tests := []struct {
		...
		{method: DirectDownload, substr: "download"},
		{method: Homebrew, substr: "brew upgrade"},
		{method: GoInstall, substr: "go install"},
		{method: PackageManager, substr: "apt"},
		{method: BuildFromSource, substr: "git pull"},
		{method: Unknown, substr: "Unable to determine"},
	}
	for _, tt := range tests {
		t.Run(string(tt.method), func(t *testing.T) {
			got := Guidance(tt.method, "tool")
			if got == "" {
				t.Fatalf("Guidance(%q) = empty string", tt.method)
			}
			if !stringsContains(got, tt.substr) {
				t.Fatalf("Guidance(%q) = %q, want it to contain %q", tt.method, got, tt.substr)
			}
		})
	}
}
