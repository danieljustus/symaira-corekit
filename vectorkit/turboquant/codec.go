...
// Errors
var (
	ErrDimMismatch     = errors.New("turboquant: dimension mismatch")
	ErrInvalidBitWidth = errors.New("turboquant: unsupported bit width")
	ErrCodeTooShort    = errors.New("turboquant: code bytes too short")
	ErrMetaTooShort    = errors.New("turboquant: metadata bytes too short")
	ErrMetaVersion     = errors.New("turboquant: unsupported metadata version")
)
...
// ---------------------------------------------------------------------------
// Sidecar integration — a self-describing layout a consumer can persist
// alongside its own records (for example a database sidecar column).
//
...
// SidecarMeta carries JSON tags so the consumer can serialise it directly
// into its own metadata column; this package owns neither the storage schema
// nor any database dependency.
// ---------------------------------------------------------------------------
...
// quantised sidecar. Its JSON tags define the wire format; a consumer can
// map it onto a matching storage type. This package intentionally depends on
// no storage layer, so the field tags are the contract.
...
// The returned SidecarMeta is ready for the consumer to persist as JSON.
...
var (
	ErrInvalidSidecarMeta   = errors.New("turboquant: invalid sidecar metadata")
	ErrCodecVersionMismatch = errors.New("turboquant: sidecar codec version mismatch")
	ErrSeedMismatch         = errors.New("turboquant: sidecar seed mismatch")
)
